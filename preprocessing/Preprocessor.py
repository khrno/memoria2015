...
class Preprocessor:
    ...
    def loadFromResource(self, resourceFilename):
        ...
        self.tokens = tokens
        if self.verbose:
            print "\t... %d tokens load from %s in %d seconds" % (
            len(self.tokens), resourceFilename, int(time.time() - starTime))
    ...
    def tokenizer(self):
        ...
        with open(self.input_filename) as stream:
            ...
            for line in lines:
                ...
                [tokens.append(w) for w in nltk.word_tokenize(title.lower().translate(None, string.punctuation)) if
                 self.isValidToken(w)]
    ...
    def filterByPosTag(self, pos_tag):
        ...
        print "\t... %d tokens filtered by pos tag %s in %d seconds" % (
        len(self.tokens), pos_tag, int(time.time() - starTime))
    ...
    def stemmer(self):
        ...
        print "\t...", len(self.tokens), "tokens stemmed (by " + self.stemAlgorithm + " algorithm) in", int(
            time.time() - starTime), "seconds"
    ...
    def exportTokensOrdered(self):
        ...
        fileTokens.close()
        if self.verbose:
            print "\t... %d tokens are written in %s/tokens.txt in %d seconds" % (
            len(self.tokens), self.output_path, int(time.time() - starTime))
    ...
    def exportTokenFrequency(self, threshold=100):
        ...
        if self.verbose:
            print "\t... %d tokens with frequency greather than %d ocurrences are written in %s/token_frequency.csv in %d seconds" % (
            cont, threshold, self.output_path, int(time.time() - starTime))
